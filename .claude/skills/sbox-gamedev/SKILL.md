...
- [rules/scenes.md](rules/scenes.md) — `.scene` JSON format, quaternion rotation cheatsheet, component field shapes (Camera/ModelRenderer/ScreenPanel/BoxCollider/DirectionalLight), scene-load gotchas. Also: **never `EditorScene.OpenScene` via eval after launch** — the editor restores the open scene + dock/viewport layout natively, and force-opening resets the user's layout; toggle/edit the live scene via eval instead of file-edit + relaunch. **Read this when authoring or modifying scenes from a script.**
...
- [rules/resources.md](rules/resources.md) — `GameResource` custom data-driven content (declaring `[GameResource("Name", "ext", ...)]`, `.ext` JSON file shape with `Color`/`Model` text encodings, `ResourceLibrary.Get<T>`/`GetAll<T>`), Save/Load via `FileSystem.Data` + `JsonSerializer`, and the gotcha that `sbox-eval` can't see `Local.<X>` namespaces directly — must reflect via `AppDomain.GetAssemblies()`. **Read when adding data-driven content (themes, weapons, levels, item defs).**
- [rules/navigation.md](rules/navigation.md) — `Scene.NavMesh.Generate()` (runtime bake), `NavMeshAgent` Component setup + scene-file shape, `MoveTo`/`Stop` API, sampling random/closest points, animgraph integration from agent velocity. **Critical: the type is `Sandbox.Navigation.NavMesh`, not `Sandbox.NavMesh`** (latter doesn't exist — burns hours if you guess wrong). Also: `NavMeshAgent.UpdateRotation` aims at a path look-ahead (spins toward blocked targets, snaps ~180° at arrival) — set it false and face velocity yourself. **Read when adding any AI/pathfinding.**
- [rules/shaders.md](rules/shaders.md) — custom `.shader` HLSL files DO compile on Wine/Linux (verified), `.vmat` Material file format and editor renormalization, `Material.Load` / `MaterialOverride` runtime swap, HDR `Tint` + `PostProcessVolume` for glow without writing shaders, hover/selection outline via `HighlightOutline` + camera `Highlight`. **Critical: `CameraComponent.EnablePostProcessing` defaults to FALSE — NO post-process (outline/bloom/tonemap) renders until it's true, and the failure is silent.** Also: editor pre-cache scans description strings as asset paths — avoid `.vmat`/`.vfx`/`.vmdl` substrings in user-visible [Property] strings. **Read when authoring or driving custom visuals.**
- [rules/terrain.md](rules/terrain.md) — `Sandbox.Terrain` Component + `TerrainStorage` GameResource, .terrain JSON file format with raw-Deflate (wbits=-15) compressed ushort heightmap, custom `.tmat` materials must be in-project (engine sidecar BCR/NHO textures don't always load), `Scene.Trace.Ray` works for ground sampling because Terrain inherits Collider, `TerrainScatterer` is editor-only (scatter clutter manually at runtime), `VolumetricFogVolume` setup. **Read when building outdoor environments.**
- [rules/cinematics.md](rules/cinematics.md) — `MovieMaker.MoviePlayer` for runtime camera animation (build `MovieClip` in code with separate Position/Rotation/Scale tracks, NOT a fused Transform; `CreateTargets=false` to avoid duplicates), ActionGraph is editor-only — pivot to C# state machines, Localization JSONs work but runtime language switching is blocked (`Application.LanguageCode` internal-set + whitelist; use a parallel C# dictionary). **Read for menus, intros, scripted camera moves.**
- [rules/networking.md](rules/networking.md) — `[Sync]` properties, `[Sync(SyncFlags.FromHost)]` for host-authoritative state, `[Rpc.Broadcast]`/`[Rpc.Host]`/`[Rpc.Owner]` patterns, `Networking.IsHost` gating, the gotcha that `[Rpc.Broadcast]` body runs on the host too (use `NetFlags.HostOnly` to exclude). Single-process simulation is a valid demo path — set `GameNetworkType=Multiplayer` in .sbproj and the codegen runs even without remote peers. **Read for multiplayer or for writing code that will network correctly later.**
- [rules/character-customization.md](rules/character-customization.md) — `Clothing` GameResource format, `ClothingContainer.Add/Toggle/Apply` pipeline, bodygroups (`SetBodyGroup(name, value)`) for hiding head/chest, morphs (often empty on default citizen), and the multiplayer pattern: `Connection.GetUserData("avatar")` returns the Steam-menu avatar JSON for `ClothingContainer.Deserialize`. **Read for any avatar/wardrobe content.**
- [rules/cloud-assets.md](rules/cloud-assets.md) — community / sbox.game packages (ident `<org>.<name>`): fetch + **mount** them to disk via `Package.FetchAsync`/`MountAsync` (through `sbox-eval`), where the files land (`download/assets/`, structured vs flat content-addressed, compiled `.vmdl_c`/`.clothing_c` with the source JSON embedded), decompiling them with VRF, and feeding community clothing into the kimodo `/kata` viewer. **Read to use any community-published model/clothing.**
- [rules/npc-framework.md](rules/npc-framework.md) — layered NPC pattern (Npc + SensesLayer + NavigationLayer + AnimationLayer + SpeechLayer; ScheduleBase + TaskBase selector). `Sandbox.Npcs.Npc` is unreachable from external projects (sandbox is Type=game); mirror the pattern locally. **Critical: `Scene.FindInPhysics` misses CharacterControllers** — enumerate by tag instead. Tag inheritance from parent means child bones look like actors — require `go.Root == go`. **Read when building any AI character (guard, enemy, companion).**
- [rules/animations.md](rules/animations.md) — citizen model + animgraph parameter reference (the 16 you actually need out of 75 the model exposes), local-space velocity computation for the 8-way locomotion blend, edge-triggered jump pattern, sit param set (`b_grounded` required or it "squats"; there's no `b_sit`) + procedural seat alignment, root motion from sequences (`ExtractMotion` → `RootMotion`, never rescale it), `CharacterController` is *not* a `Collider`, footstep timing without anim events, 3rd-person camera follow pattern. **Read when building any character-driven game.**
...
- `sbox-recompile` — trigger an in-editor recompile via `Sandbox.Project.CompileAsync()` reflection. **Takes ~0.2s** vs ~35s for kill+relaunch. Use whenever you only edited existing source files. **Now greps the editor log for the compile-result line and exits non-zero with diagnostics if it failed** — no more silent "RanToCompletion + stale assembly" stalls. See `rules/debugging.md` → "When to relaunch vs. when to recompile".
...
All worked examples live under `<repo>/examples/`. Skim several when starting a new project — each one covers different patterns:

| Game | Demonstrates |
|---|---|
| `examples/coin-rush/` | basic Component + scene + HUD scaffold, scene generated from Python |
| `examples/flappy-bird/` | side-scroller + game states (Menu/Playing/GameOver) + Razor HUD |
| `examples/pool/` | Rigidbody + Collider physics, sound-on-event, trigger pockets, mouse drag aim |
| `examples/parkour/` | CharacterController, citizen animgraph parameters, 3rd-person camera follow |
| `examples/shooting-gallery/` | First-person mouse-look, hitscan via `Scene.Trace.Ray`, target hit/respawn |
| `examples/bullet-hell/` | Top-down arcade prototype with multiple enemy/bullet types |

Plus `<repo>/model-probe/` (kept at the repo root, NOT under examples/) — a worker project used by `sbox-model-info` as a fallback when no editor is running. Don't delete it.

New examples go under `examples/<name>/` to keep the repo root clean.
